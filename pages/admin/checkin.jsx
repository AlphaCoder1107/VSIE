              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4 grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <button onClick={()=>setScanning(true)} disabled={scanning} className="px-3 py-2 rounded-lg bg-vsie-accent text-white disabled:opacity-50">Start scanning</button>
                  ...
                  {result && (
                    <div className="mt-4 text-sm bg-white/5 border border-white/10 rounded-lg p-3">
                      <div><span className="text-white/60">Code:</span> <span className="font-mono">{result.registration_code}</span></div>
                      <div><span className="text-white/60">Name:</span> {result.student_name}</div>
                      <div><span className="text-white/60">Checked in:</span> {String(result.checked_in)}</div>
                      {result.checked_in_at && <div><span className="text-white/60">At:</span> {new Date(result.checked_in_at).toLocaleString()}</div>}
                    </div>
                  )}
                </div>
                <div>
